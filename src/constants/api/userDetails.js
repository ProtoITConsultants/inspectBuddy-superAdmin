...
const UPDATE_SPECIFIC_ROOM_IN_TEMPLATE_URL = ({ userId }) =>
  `/templateUpdateRoom?id=${encodeURIComponent(userId)}`;
...
const DELETE_SAVED_QUESTION = ({ userId, questionId }) =>
  `/deleteSavedQuestion/${encodeURIComponent(
    questionId
  )}?id=${encodeURIComponent(userId)}`;
...
const USER_DETAILS_ENDPOINTS = {
  ...
  UPDATE_SPECIFIC_ROOM_IN_TEMPLATE_URL,
